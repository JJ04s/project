...
def show_conclusion():
    ...
    st.markdown(
        ...
        출발역, 상선/하선 여부, 요일구분(평일, 토요일, 일요일) 데이터를 포함하며 시간대, 역별 혼잡도 정보를 제공한다.
        혼잡도 기준은 열차 정원 대비 실제 승차인원 비율로 계산된다.

        -서울시 지하철 시간별 승하차 인원 정보

        사용월 데이터를 포함하며 시간대, 역 별 승차, 하차인원 데이터를 제공한다.

        - 서울시 지하철역 엘리베이터 위치 정보

        역 별 엘리베이터 위치 데이터를 제공한다.
        본 프로젝트에서는 이를 지하철역의 위치로써 이용하였다.

        - 도시철도 전체노선정보

        호선정보, 역 순서 정보, 역명 데이터 등을 포함한다.
        xml 형태의 데이터로 제공되어 웹크롤링을 통해 여러 호선에 대한 정보를 수집 후 결합하였다.


        본 프로젝트에서는 각 데이터셋을 시간대와 역 단위로 정렬·결합하여, 
        시간대별 혼잡도 변화와 승하차 흐름을 분석하는 데 활용하였다.
        """)
